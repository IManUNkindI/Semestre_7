...
int recordedAudio[SAMPLE_WINDOW / SAMPLE_DELAY]; // Arreglo para almacenar las lecturas del micrófono
...
void loop() {
  ...
  if (playAudio) { // Si se va a reproducir el audio
    ...
    if (client.connect(host, 1233)) { // Conexión al servidor
      Serial.println("Conectado al servidor");
      client.println("Hola desde ESP32 - 1"); // Mensaje a enviar
      client.stop(); // Cierra la conexión
    } else {
